  ...
  <div class="p-6 bg-gray-50 min-h-screen">
    ...
    <div v-else-if="user && user.role_id === 2">
      <div class="flex justify-between items-center mb-3">
        ...
        <button
          class="bg-purple-500 text-white px-4 py-2 rounded-xl hover:bg-purple-600 shadow-md"
...
import RolesDialog from "../../page/dialogs/RoleDialog.vue"; 
...
export default defineComponent({
  components: { EditUserDialog, RolesDialog },
  setup() {
    ...
    const fetchRolesData = async (): Promise<void> => {
      try {
        const response = await fetch('/api/roles');
        const data = await response.json();
        rolesDataList.value = data;
      } catch (error) {
